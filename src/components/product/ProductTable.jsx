...
const ProductTable = () => {
  ...
  const columns = useMemo(() => [
    ...
    { id: 'value', header: 'Total Value', cell: ({ row }) => `$${((row.original.stock || 0) * (row.original.price || 0)).toFixed(2)}` },
    {
      ...
      cell: ({ row }) => (
        <Box>
          <IconButton size="small" onClick={() => console.log('Edit', row.original)}><Edit /></IconButton>
          <IconButton
            size="small"
            onClick={() => {
  ...
  return (
    <Box sx={{ p: 2 }}>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
        ...
        <Button variant="contained" onClick={() => setIsModalOpen(true)}>Add New Product</Button>
      </Box>
